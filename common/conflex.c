...
Author:  	  wangzhe
Description:  将文件解析到struct parse结构
...
)
{
	...
	else 
	{
		...
		/* 通过已经打开的文件描述符file获取文件状态，返回值0是成功 */
		if (fstat(file, &sb) < 0) 
		{
		...
		/* mmap，将一个文件映射到内存，inbuf指向这个内存的起始地址 */
		tmp->bufsiz = tmp->buflen = (size_t)sb.st_size;
		tmp->inbuf = mmap(NULL, tmp->bufsiz, PROT_READ, MAP_SHARED, file, 0);
	...
	if (!cfile->ugflag) 
	{
		...
		else if (c != EOF) 
		{
			...
			if (cfile->lpos <= 80)
			{
				...
				cfile->cur_line[cfile->lpos] = 0;		//用0标识当前位置
			}
			cfile->lpos++;
		}
	}
 ...
 */
/* 我们只想看到非空格符号，除非一些特殊情况确实需要空格，一般用next_token()读取下一个token，
这个函数调用get_next_token，不会返回空格，而是跳过空格，当需要空格的时候，使用next_raw_token，
这个函数会把空格也作为符号返回。peek_token和peek_raw_token一样，注意，若先使用peek_token，
然后接下来对next_raw_token和peek_raw_token的使用也会丢弃空格，因为peek_token会消耗空格 */
static enum dhcp_token get_raw_token
(
	struct parse *cfile
) 
{
...
Date Created: 2018/06/07
Author:  	  wangzhe
Description:  解析符号并返回，此函数只是peek符号，不消耗符号
Input:	      
Output:       
Return:       
Caution : 	  The do_peek_token() function checks the next token without
...
/* 从cfile中读取下一个符号并将这个符号返回，留给接下来的next_token()调用，此函数不消耗符号，
但是会消耗空格 */
enum dhcp_token peek_token
(
	const char **rval, 			//解析到的字符串保存在这里
	unsigned *rlen, 
	struct parse *cfile
) 
{
...
/*********************************************************************
Func Name :   read_whitespace
Date Created: 2018/06/07
Author:  	  wangzhe
Description:  读取尽可能多的空格，直到遇到一个不是空格的字符
Input:	      
Output:       
Return:       
Caution : 	  
*********************************************************************/
static enum dhcp_token read_whitespace
(
	int c, 
	struct parse *cfile
) 
{
	...
	do {
		...
		if (c == EOF)
			return END_OF_FILE;
	} while (!((c == '\n') && cfile->eol_token) && isascii(c) && isspace(c));
...
Date Created: 2018/06/07
Author:  	  wangzhe
Description:  读取一个字符串到cfile->tokbuf
Input:	      IN struct parse * cfile
Output:       
Return:       
Caution : 	  
*********************************************************************/
static enum dhcp_token read_string
(	
	struct parse *cfile
)
{
...
/*********************************************************************
Func Name :   read_number
Date Created: 2018/06/08
Author:  	  wangzhe
Description:  读取数字到cfile->tokbuf
Input:	      IN int c			起始数字
			  IN struct parse * cfile
Output:       
Return:       
Caution : 	  
*********************************************************************/
static enum dhcp_token read_number
(	
	int c,
	struct parse *cfile
)
