...
struct timeout *timeouts;		//defined in dhcpd.h
...
/*********************************************************************
Func Name :   process_outstanding_timeouts
Date Created: 2018/05/24
Author:  	  wangzhe
Description:  Call any expired timeouts, and then if there's
	   			still a timeout registered, time out the select call then
Input:	      IN struct timeval * tvp
Output:       
Return:       struct timeval *
Caution : 	  要求timeouts链表是按照时间从小到大排序
*********************************************************************/
struct timeval *process_outstanding_timeouts (struct timeval *tvp)
{
	...
	{
		struct timeout *t;
		/* 若timeouts->when的时间比cur_tv的少，说明过期了 */
		if ((timeouts->when.tv_sec < cur_tv.tv_sec) || ((timeouts->when.tv_sec == cur_tv.tv_sec) &&
...
/*********************************************************************
Func Name 	 : add_timeout
Date Created : 2018/06/05
Author		 : wangzhe
Description	 : 设置一个定时器
Input		 : IN struct timeval * when
			   IN void (*where)(void *)		超时调用函数
			   IN void * what				数据，比如lease
			   IN tvref_t ref
			   IN tvunref_t unref
Output		 : 
Return		 : void
