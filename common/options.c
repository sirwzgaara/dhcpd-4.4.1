...
Output:       None
Return:       
Caution : 	  Parse all available options out of the specified packet.
...
)
{
	...
	/* 这里的magic number是八进制定义的，十进制是，99，130，83，99 */
	if (memcmp(packet->raw->options, DHCP_OPTIONS_COOKIE, 4)) 
	{
		/* 若没找到magic number，valid标记置0 */
				  ...
				  (packet->packet_length - DHCP_FIXED_NON_UDP - 4), &dhcp_universe)) 
	{
		...
		if ((op = lookup_option(&dhcp_universe, packet->options, DHO_DHCP_MESSAGE_TYPE))) 
		{
			if (!op->data.data ||
	...
	if ((op = lookup_option(&dhcp_universe, packet->options, DHO_DHCP_OPTION_OVERLOAD))) 
	{
		if (op->data.data[0] & 1) 
		{
			if (!parse_option_buffer(packet->options, (unsigned char *)packet->raw->file,
			     sizeof(packet->raw->file), &dhcp_universe))
			{
				return 0;
			}
		}
		if (op->data.data[0] & 2) 
		{
			if (!parse_option_buffer(packet->options, (unsigned char *)packet->raw->sname,
			     sizeof(packet->raw->sname), &dhcp_universe))
			{
				return 0;
			}
		}
...
Input:	      OUT struct option_state * options	  要解析到这个字段
	`		  IN const unsigned char * buffer     options字段除去魔数后起始地址
			  ...
			  IN struct universe * universe		  dhcp_universe
Output:       None
Return:       void
Caution : 	  Parse options out of the specified buffer, storing addresses of option
 			 	values in packet->options	
*********************************************************************/
int parse_option_buffer 
(
...
)
{
	...
	/* length是纯options字段的长度，一般discover报文是13，这样分配之后长了一个字节 */
	...
	/* dhcp_universe的get_tag是取字符串的第一个字节，tag_size是1 */
	for (offset = 0; (offset + universe->tag_size) <= length &&
	     (code = universe->get_tag(buffer + offset)) != universe->end;) 
	{
		...
		{
			reason = "code tag at end of buffer - missing length field";
			goto bogus;
		}
		...
		/* hash_lookup，若找到的话，增加引用计数，code_hash是dhcp_universe的code的hash表 */
		/* 这里是能找到的，找到后，option指向dhcp_options[i]全局变量 */
		...
		*/
		...
		if (op == NULL) 
		{
			...
			if (save_option_buffer(universe, options, bp, bp->data + offset, len,
					       code, 1) == 0) 
			{
				log_error("parse_option_buffer: save_option_buffer failed");
				buffer_dereference(&bp, MDL);
				return (0);
			}
		} 
		else if (universe->concat_duplicates) 
		{
			...
			if (!buffer_allocate(&new.buffer, op->data.len + len, MDL)) 
			{
			...
			memcpy(new.buffer->data, op->data.data, op->data.len);
			/* Concat new option behind old. */
			memcpy(new.buffer->data + op->data.len, bp->data + offset, len);
...
/*********************************************************************
Func Name :   lookup_option
Date Created: 2018/05/23
Author:  	  wangzhe
Description:  
Input:	      IN struct universe
			  IN struct option_state * options
			  unsigned code
Output:       
Return:       struct option_cache *
Caution : 	  	
*********************************************************************/
struct option_cache *lookup_option 
(
	struct universe * universe, 
	struct option_state * options, 
	unsigned code
)
{
	...
	if (universe->lookup_func)
		...
		log_error ("can't look up options in %s space.", universe - name);
	
	return (struct option_cache *)0;
}

/*********************************************************************
Func Name :   lookup_hashed_option
Date Created: 2018/05/23
Author:  	  wangzhe
Description:  在option_state的universe数组的hash表中查找code
Input:	      IN struct universe * universe		全局universe比如dhcp_unvierse
			  IN struct option_state * options	要解析到的option
...
)
{
	...
	/* 这些内存在给option分配内存的时候一起分配了 */
	if (universe->index >= options->universe_count || !(options->universes[universe->index]))
		return (struct option_cache *)0;

	/* 指针数组每个格子指向一个hash表，hash指向index对应的那个hash表 */
...
Output:       
Return:       int
Caution : 	  Create/copy a buffer into a new option cache
*********************************************************************/
static int prepare_option_buffer
(
	struct universe *universe, 
	struct buffer *bp,
	unsigned char *buffer, 
...
)
{
	...
	{
		log_error("No memory for option code %s.%s.", universe->name, option->name);
		status = 0;
		goto cleanup;
	}
...
Date Created: 2018/05/21
Author:  	  wangzhe
Description:  判断请求包的类型是BOOTP或DHCP，调用各自处理函数
Input:	      IN struct interface_info * interface		接口信息
			  OUT struct dhcp_packet * packet			报文中的dhcp部分内容
			  IN unsigned len							从dhcp内容开始的报文长度
			  IN unsigned int from_port					包发来的源port
			  IN struct iaddr from						包发来的源IP，包括长度和地址
			  IN struct hardware * hfrom				包发来的硬件地址
Output:       
Return:       void
Caution : 
*********************************************************************/
...
)
{
	...
	/* DHCP_FIXED_NON_UDP是DHCP报文固定部分长度236字节，4是magic number的4个字节 */
	/* length应该是253，295-14-20-8，是DHCP内容的长度 */
	if (decoded_packet->packet_length >= DHCP_FIXED_NON_UDP + 4) 
	{
		...
		if (decoded_packet->options_valid &&
		    (op = lookup_option(&dhcp_universe, decoded_packet->options, DHO_DHCP_MESSAGE_TYPE))) 
		{
			struct data_string dp;
			memset(&dp, 0, sizeof(dp));
			evaluate_option_cache(&dp, decoded_packet, NULL, NULL,
					      decoded_packet->options, NULL, NULL, op, MDL);
			if (dp.len > 0)
			{
				/* 保存data的时候已经越过了标识符和长度，就是value */
	...
	{
		/* 若报文类型字段不为空，说明是dhcp报文，进入dhcp模块处理 */
