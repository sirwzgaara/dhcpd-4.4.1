...
/*********************************************************************
Func Name :   subnet_numbet
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  Return just the network number of an internet address...
Input:	      struct iaddr addr
			  struct isddr mask
Output:       
Return:       struct iaddr
Caution : 	  
*********************************************************************/
struct iaddr subnet_number 
(
	struct iaddr addr, 
	struct iaddr mask
)
...
Author:  	  wangzhe
Description:  拼装子网号和主机号组成一个网络地址
Input:	      IN struct iaddr subnet		子网地址
			  IN struct iaddr mask			掩码
			  IN u_int32_t host_address		主机号
Output:       
Return:       struct iaddr
Caution : 	  Combine a network number and a integer to produce an internet address.
...
)
{
	...
	if (subnet.len != mask.len)
		log_fatal("ip_addr():%s:%d: Addr/mask length mismatch.", MDL);
...
Date Created: 2018/06/02
Author:  	  wangzhe
Description:  返回一个网络地址的主机号
Input:	      IN struct iaddr addr
			  IN struct iaddr mask
Output:       
Return:       void
Caution : 	  返回的是大小端转换后的结果
*********************************************************************/
u_int32_t host_addr 
(
	struct iaddr addr, 
...
)
{
	...
	if (addr.len != mask.len)
		log_fatal("host_addr():%s:%d: Addr/mask length mismatch.", MDL);
...
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  判断两个网络地址是否一样
Input:	      IN struct iaddr addr1
			  IN struct iaddr addr2
Output:       
Return:       int
Caution : 	  
*********************************************************************/
