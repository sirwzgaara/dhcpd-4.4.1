...
/*********************************************************************
Func Name :   decode_ethernet_header
Date Created: 2018/06/06
Author:  	  wangzhe
Description:  解析以太网二层头
Input:	      IN struct interface_info * interface	接口信息
   			  IN unsigend char * buf		存报文的buffer
   			  IN unsigned bufix				包偏移
   			  IN struct hardware * from		包发来的硬件地址
Output:       
Return:       ssize_t
Caution :     
*********************************************************************/
ssize_t decode_ethernet_header 
(
	struct interface_info *interface,
	unsigned char *buf,
	unsigned bufix,
