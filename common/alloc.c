...
/*********************************************************************
Func Name :   group_allocate
Date Created: 2018/06/08
Author:  	  wangzhe
Description:  给struct group分配内存
Input:	      IN 
Output:       IN struct group **ptr
			  IN const char * file
			  IN int line
Return:       
Caution : 
...
/*********************************************************************
Func Name :   option_cache_allocate
Date Created: 2018/06/06
Author:  	  wangzhe
Description:  申请一个option_cache结构的内存
Input:	      
Output:       
Return:       int
Caution : 	  
*********************************************************************/
int option_cache_allocate 
(
	...
	int line;
{
	...
	*ptr = dmalloc(size, file, line);
	if (*ptr) {
		...
		(*ptr)->refcnt = 1;
		rc_register(file, line, ptr, *ptr, (*ptr)->refcnt, 0, RC_MISC);
		return 1;
	}
	return 0;
}
...
Date Created: 2018/06/06
Author:  	  wangzhe
Description:  拷贝一个data_string
Input:	      
Output:       
Return:       
Caution : 	  
*********************************************************************/
void data_string_copy
(
