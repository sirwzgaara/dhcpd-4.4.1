...
/*********************************************************************
Func Name 	 : icmp_startup
Date Created : 2018/06/13
Author		 : wangzhe
Description	 : 初始化ICMP支持
Input		 : IN int routep
			   IN void (*handler)(struct iaddr, u_int8_t *, int)
Output		 : 
Return		 : void
Caution 	 : Initialize the ICMP protocol, handler = lease_pinged
*********************************************************************/
void icmp_startup
(
	int routep,
...
)
{
	...
	if (result != ISC_R_SUCCESS)
		log_fatal ("Can't register icmp object type: %s", isc_result_totext(result));
...
/*********************************************************************
Func Name 	 : icmp_echorequest
Date Created : 2018/06/14
Author		 : wangzhe
Description	 : 给一个IP地址发送ping包
Input		 : IN struct iaddr * addr
Output		 : 
Return		 : 
Caution 	 : 
*********************************************************************/
int icmp_echorequest
(
	struct iaddr *addr
)
{
	...
	icmp.icmp_cksum = wrapsum(checksum((unsigned char *)&icmp, sizeof(icmp), 0));
