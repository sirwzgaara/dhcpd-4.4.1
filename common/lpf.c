...
#ifdef USE_LPF_RECEIVE
/*********************************************************************
Func Name :   receive_packet
Date Created: 2018/05/30
Author:  	  wangzhe
Description:  收包处理函数
Input:	      IN struct interface_info * interface		接口信息
			  IN unsigned char * buf					保存报文的buffer
			  IN size_t len
			  IN struct sockaddr_in * from				包发来的socket地址
...
)
{
	...
	/* 一般offset是14，源mac，目的mac，类型，共计14字节，将二层头写入ibuf，用bufix控制偏移 */
	...
	/* 一般这个offst是28，IP头20字节，UDP头8字节，填写from中的源IP和源port号 */
	...
	/* 这样操作以后，外面的union，就可以让packet指向dhcp报文内容了 */
