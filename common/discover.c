...
Author:  	  wangzhe
Description:  每个client发来的包，都进入这个函数处理
Input:	      IN omapi_object_t
Output:       None
Return:       void
Caution : 
*********************************************************************/
isc_result_t got_one 
(
	omapi_object_t * h
)
{
	struct sockaddr_in from;		//包发来的socket地址
	struct hardware hfrom;			//包发来的硬件地址
	struct iaddr ifrom;
	int result;
	union 
	{
		/* Packet input buffer. Must be as large as largest possible MTU. */
		unsigned char packbuf[4095];
		struct dhcp_packet packet;
	}u;								//是一个union，所以偏移之后，packet就是dhcp报文内容
	struct interface_info *ip;
	...
	ip = (struct interface_info *)h;
      ...
      again:
	/* 一般来说result是253，调用的是lpf.c中的这个函数 */
...
/* 这个走不进去 */
	...
	if (bootp_packet_handler) 
	{
		...
		(*bootp_packet_handler)(ip, &u.packet, (unsigned)result, from.sin_port, ifrom, &hfrom);
	}
