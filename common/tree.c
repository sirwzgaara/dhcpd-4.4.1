...
/*********************************************************************
Func Name 	 : make_const_data
Date Created : 2018/06/05
Author		 : wangzhe
Description	 : ¸øconst_data×Ö¶Î¸³Öµ
Input		 : IN struct expression ** expr
...
)
{
	...
	if (len) 
	{
		if (allocate) 
		{
			...
			nt->data.const_data.data = &nt->data.const_data.buffer->data[0];
			memcpy(nt->data.const_data.buffer->data, data, len + terminated);
		} 
		else 
...
)
{
	...
	/* ÈôÊÇdata£¬±ÈÈç×Ö·û´®»òÕßoptionÖ®ÀàµÄ */
...
Date Created: 2018/07/24
Author:  	  wangzhe
Description:  ¼ÆËãÒ»¸ö²¼¶û±í´ïÊ½µÄÖµ£¬·µ»Ø1»òÕß0±íÊ¾trueºÍfalse
Input:	      
Output:       
Return:       int
Caution : 	  
...
Author:  	  wangzhe
Description:  ´ÓoptionÖÐ½âÎövalue²¢¸³Öµµ½result
Input:	      
Output:       
Return:       
Caution : 	  Return data hanging off of an option cache structure, or if there
