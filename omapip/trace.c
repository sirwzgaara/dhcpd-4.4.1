...
/*********************************************************************
Func Name :   trace_init
Date Created: 2018/05/24
Author:  	  wangzhe
Description:  
Input:	      
Output:       
Return:       ISC_R_SUCCESS		成功
			  ISC_R_UNEXPECTED	意外
Caution : 
*********************************************************************/
			 ...
			 const char *file, int line)
{
	...
	static int root_setup = 0;		//注意这个是static的
...
/*********************************************************************
Func Name :   trace_begin
Date Created: 2018/05/24
Author:  	  wangzhe
Description:  启动追踪入口函数
Input:	      
Output:       
Return:       isc_result_t
Caution : 
			  ...
			  const char *file, int line)
{
	...
	/* 先检查traceoutfile，防止启动trace两次的情况 */
	if (traceoutfile) {
	...
	/* open第三个参数mode_t是八进制，若文件不存在，通过O_CREAT控制创建文件时，设置这个文件的权限，
	0600意思是，拥有读写权限*/
...
/*********************************************************************
Func Name :   trace_type_stash
Date Created: 2018/05/24
Author:  	  wangzhe
Description:  把tptr加到trace_types数组
Input:	      
Output:       
Return:       void
Caution : 	  **trace_types是已有的数组，trace_type_count是已有的数组
			的元素个数，trace_type_max是目前有的内存个数，若发现新到一
			个index，超过了max，那么就要重新分配内存
*********************************************************************/
void trace_type_stash (trace_type_t *tptr)
{
	...
	{
		delta = tptr->index - trace_type_max + 10; 		//保证delta比10大
		vec = dmalloc(((trace_type_max + delta) * sizeof (trace_type_t *)), MDL);
		if (!vec)
			return;
		/* 从trace_type_max开始后面申请了delta个内存，给这段内存初始化 */
		memset(&vec[trace_type_max], 0, (sizeof(trace_type_t *)) * delta);
		trace_type_max += delta;		//即trace_type_max = tptr->index + 10

		/* 将trace_type_count个trace_types拷贝到vec中 */
		if (trace_types) 
		{
		    memcpy(vec, trace_types, trace_type_count * sizeof(trace_type_t *));
		    dfree(trace_types, MDL);		//原来的废弃，直接用新的
		}
		trace_types = vec;
	}
...
/*********************************************************************
Func Name :   trace_index_map_input
Date Created: 2018/05/24
Author:  	  wangzhe
Description:  把new_trace_types链表中和buf名字一致的节点加入trace_types数组
Input:	      
Output:       
Return:       void
Caution : 
...
void trace_index_map_input (trace_type_t *ttype, unsigned length, char *buf)
{
	...
	/* 遍历new_trace_types链表，只要找到，就处理并return */
	prev = &new_trace_types;
	for (tptr = new_trace_types; tptr; tptr = tptr->next) 
	{
		len = strlen(tptr->name);
		if (len == length - TRACE_INDEX_MAPPING_SIZE && !memcmp(tptr->name, tmap->name, len)) 
		{
