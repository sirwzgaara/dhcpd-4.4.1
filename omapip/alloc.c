...
/*********************************************************************
Func Name :   omapi_object_reference
Date Created: 2018/05/22
Author:  	  wangzhe
Description:  h是已经分配好的内存，将r挂到h上，并增加h的引用计数
Input:	      IN opapi_object_t * t
			  IN const char * file
			  IN int line
Output:       OUT omapi_object_t ** r
Return:       void
Caution : 
*********************************************************************/
...
/*********************************************************************
Func Name :   omapi_object_dereference
Date Created: 2018/06/02
Author:  	  wangzhe
Description:  omapi对象解引用
Input:	      IN omapi_object_t ** h
			  IN const char * file
			  IN int line
Output:       
Return:       DHCP_R_INVALIDARG
...
Caution : 	  若没找到返回NULL
*********************************************************************/
isc_result_t omapi_object_dereference 
(
	omapi_object_t **h,
    const char *file, 
    int line
)
{
	...
	if ((*h)->refcnt == inner_reference + outer_reference + handle_reference + 1) 
	{
		if (inner_reference || outer_reference || handle_reference) 
		{
			/* XXX we could check for a reference from the handle table here. */
			extra_references = 0;
			for (p = (*h)->inner; p && !extra_references; p = p->inner) 
			{
		...
		if (!extra_references) 
		{
			...
			*h = 0;
			hp->refcnt--;
			if (inner_reference)
				omapi_object_dereference(&hp->inner, file, line);
			if (outer_reference)
				omapi_object_dereference(&hp->outer, file, line);
/*			if (!hp -> type -> freer) */
			rc_register (file, line, h, hp, 0, 1, hp->type->rc_flag);
			if (handle_reference) 
			{
				if (omapi_handle_clear(hp->handle) != ISC_R_SUCCESS) 
				{
					log_debug("Attempt to clear null handle pointer");
				}
			}
			if (hp->type->destroy)
		...
		else 
		{
			(*h)->refcnt--;
/*			if (!(*h) -> type -> freer) */
			rc_register(file, line, h, *h, (*h)->refcnt, 1, (*h)->type->rc_flag);
		}
	} 
	else 
	{
