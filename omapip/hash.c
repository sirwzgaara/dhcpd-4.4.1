...
/*********************************************************************
Func Name :   new_hash_table
Date Created: 2018/05/23
Author:  	  wangzhe
Description:  分配hash表内存，桶深是count，采用分离链接法
Input:	      
Output:       None
Return:       void
Caution : 	  	
...
/*********************************************************************
Func Name :   new_hash_bucket
Date Created: 2018/05/23
Author:  	  wangzhe
Description:  分配一个新桶
Input:	      
Output:       
Return:       struct hash_bucket *
Caution : 	  free_hash_buckets是hash桶管理容器
*********************************************************************/
	...
	int line;
{
	...
	{
		rval = dmalloc(127 * sizeof (struct hash_bucket), file, line);
		if (!rval)
			return rval;
# if defined (DEBUG_MEMORY_LEAKAGE_ON_EXIT)
		...
		rval++;
#endif
		/* 这样操作之后，free_hash_buckets是一个127个节点的头 */
		for (; i < 127; i++) 
		{
...
)
{
	...
	{
		/* 若找到了 */
		if (len == bp->len && !(*table->cmp)(bp->name, key, len)) 
		{
			if (table->referencer)
			{
				(*table->referencer)(vp, bp->value, file, line);
			}
			else
			{
				*vp = bp->value;
