...
Author:  	  wangzhe
Description:  dhcp入口函数
Input:	      struct packet * packet
Output:       None
Return:       void
Caution : 	  此函数之前，已经完成解包
*********************************************************************/
void dhcp(struct packet *packet)
{
	     ...
	     packet->options->universes[agent_universe.index] == NULL))
	{
		...
		/* If there are no agent options on the lease, it's not interesting. */
		...
		/* 挂接agent_universe到packet->option的universes数组 */
...
/*********************************************************************
Func Name 	 : dhcpdiscover
Date Created : 2018/05/18
Author		 : wangzhe
Description	 : 处理discover报文
...
)
{
	...
	if (!lease) 
	{
		if (!allocate_lease(&lease, packet, packet->shared_network->pools, &peer_has_leases)) 
		{
			if (peer_has_leases)
			{
			...
			{
				log_error("%s: network %s: no free leases", msgbuf, packet->shared_network->name);
			}
	...
	{
		peer = lease->pool->failover_peer;
		if (peer->service_state == not_responding || peer->service_state == service_startup) 
		{
	...
	if (peer && (peer->service_state == cooperating) && !load_balance_mine(packet, peer)) 
	{
		if (peer_has_leases) 
		{
		...
		{
			log_debug("%s: cancel load balance to peer %s - %s", msgbuf, peer -> name, "no free leases");
		}
	}
#endif
	...
	ack_lease(packet, lease, DHCPOFFER, when, msgbuf, ms_nulltp, (struct host_decl *)0);
      out:
	if (lease)
	{
		lease_dereference (&lease, MDL);
...
/*********************************************************************
Func Name 	 : dhcprequest
Date Created : 2018/06/19
Author		 : wangzhe
Description	 : 处理request报文
Input		 : IN struct packet * packet
			   IN int ms_nulltp
			   IN struct lease * ip_lease
Output		 : 
Return		 : void
Caution 	 : 
*********************************************************************/
...
)
{
	...
	oc = lookup_option(&dhcp_universe, packet->options, DHO_DHCP_REQUESTED_ADDRESS);
	memset(&data, 0, sizeof data);
	if (oc &&
...
)
{
	...
	if (oc && evaluate_boolean_option_cache(&ignorep, packet, lease,
                                                NULL, packet->options, in_options,
                                                (lease ? &lease->scope : NULL), oc, MDL)) 
	{
		...
		if (oc && evaluate_option_cache(&client_id,
			...
			if (option_cache_allocate(&oc, MDL)) 
			{
				if (make_const_data(&oc->expression,
						    client_id.data,
						    client_id.len, 1, 0, MDL)) 
                {
					option_code_hash_lookup(&oc->option,
							        dhcp_universe.code_hash, &opcode, 0, MDL);
...
/*************************************************************************************/
/*************** 通过lookup_option检查一系列dhcpuniverse和server option **************/
/*************************************************************************************/

	/* See if we got a server identifier option. */
	if (lookup_option (&dhcp_universe, packet->options, DHO_DHCP_SERVER_IDENTIFIER))
	...
	/* 处理最小时间字段 */
	/* Make sure this packet satisfies the configured minimum number of seconds. */
	memset(&d1, 0, sizeof d1);
	if (offer == DHCPOFFER &&
	...
	if (!host &&
	    (oc = lookup_option(&server_universe, state->options, SV_BOOT_UNKNOWN_CLIENTS)) &&
	    !evaluate_boolean_option_cache (&ignorep,
					    packet, lease,
					    (struct client_state *)0,
					    packet -> options,
	...
	if (!offer &&
	    (oc = lookup_option(&server_universe, state->options, SV_ALLOW_BOOTP)) &&
	    !evaluate_boolean_option_cache (&ignorep,
					    packet, lease,
					    (struct client_state *)0,
					    packet->options,
	...
	/* Use the ip address of the lease that we finally found in the database. */
	...
	if (offer) 
	{
		...
		if ((oc = lookup_option (&dhcp_universe, packet->options, DHO_DHCP_LEASE_TIME)))
		...
		if (s1 && (d1.len == 4)) 
		{
						...
						oc, MDL)) 
			{
				...
				if (!ignorep)
					log_info("Infinite-leasetime reservation made on %s.",
						 piaddr(lt->ip_addr));
			}
		...
		if ((oc = lookup_option (&server_universe, state->options, SV_MAX_LEASE_TIME))) 
		{
			if (evaluate_option_cache(&d1, packet, lease,
						   (struct client_state *)0,
						   packet->options,
		...
		if (lease_time < 0 || lease_time > max_lease_time)
		...
		if ((oc = lookup_option(&server_universe, state->options, SV_MIN_LEASE_TIME))) 
		{
			if (evaluate_option_cache (&d1, packet, lease,
						   (struct client_state *)0,
						   packet->options,
		...
		if (lease->pool && lease->pool->valid_from && cur_time < lease->pool->valid_from) 
		{
		...
		else 
		{
			...
			state->offered_expiry = leaseTimeCheck(cur_time + lease_time,
						 MAX_TIME - 1);
		}
		...
		/* Don't make lease active until we actually get a DHCPREQUEST. */
		if (offer == DHCPACK)
	...
	oc = lookup_option(&server_universe, state->options, SV_IGNORE_CLIENT_UIDS);
	if ((oc == NULL) ||
	...
	/* 给lt的host，subnet，billing_class赋值 */
	if (host) 
	{
	...
	if (lease->agent_options)
		option_chain_head_reference(&lt->agent_options, lease->agent_options, MDL);
				  ...
				  NULL, &lt->scope, oc, MDL)) 
	{
		if (d1.len != 0) 
		{
			bind_ds_value(&lt->scope, "vendor-class-identifier", &d1);
		}
	...
	if (!packet->agent_options_stashed &&
	    ...
	    packet->options->universes[agent_universe.index] != NULL) 
	{
	    oc = lookup_option(&server_universe, state->options, SV_STASH_AGENT_OPTIONS);
	    if (!oc ||
			evaluate_boolean_option_cache (&ignorep, packet, lease,
					       (struct client_state *)0,
					       packet->options,
	...
	else 
	{
		...
		/* If dhcp-cache-threshold is enabled, see if "lease" can be reused. */
	...
	/* Get the Maximum Message Size option from the packet, if one was sent. */
	oc = lookup_option(&dhcp_universe, packet->options, DHO_DHCP_MAX_MESSAGE_SIZE);
	if (oc &&
	    evaluate_option_cache (&d1, packet, lease,
				   (struct client_state *)0,
				   packet->options, state->options,
	...
	{
		oc = lookup_option(&dhcp_universe, state->options, DHO_DHCP_MAX_MESSAGE_SIZE);
		if (oc &&
		    evaluate_option_cache(&d1, packet, lease,
					   (struct client_state *)0,
					   packet->options, state->options,
	...
	/* Get the Subnet Selection option from the packet, if one was sent. */
	if ((oc = lookup_option(&dhcp_universe, packet->options, DHO_SUBNET_SELECTION))) 
	{
		/* Make a copy of the data. */
	...
	/* Now, if appropriate, put in DHCP-specific options that override those. */
	if (state->offer) 
	{
		i = DHO_DHCP_MESSAGE_TYPE;
		oc = (struct option_cache *)0;
		...
		get_server_source_address(&from, state->options, state->options, packet);
		...
		/* renewing time是50%，rebingding time是87.5% */
		if ((oc = lookup_option(&dhcp_universe, state->options,
			...
			if (rebind_time >= offered_lease_time)
				delete_option(&dhcp_universe, state->options, DHO_DHCP_REBINDING_TIME);
					  ...
					  &lease->scope, oc, MDL)) 
		{
			if (getULong(d1.data) >= offered_lease_time)
				delete_option(&dhcp_universe, state->options, DHO_DHCP_RENEWAL_TIME);
	...
	if ((oc =
	     lookup_option(&server_universe, state->options, SV_NEXT_SERVER))) 
	{
		if (evaluate_option_cache(&d1, packet, lease,
					   (struct client_state *)0,
					   packet->options, state->options,
...
/*********************************************************************
Func Name 	 : dhcp_reply
Date Created : 2018/06/05
Author		 : wangzhe
Description	 : 将包发送给client
...
)
{
	...
	struct dhcp_packet raw;
	struct sockaddr_in to;			//通信地址，用于发包
	struct in_addr from;
	struct hardware hto;			//硬件地址，用于发包
	int result;
	...
	/* ciaddr是client希望的IP，yiaddr是server分配的 */
	...
	/* Make sure outgoing packets are at least as big as a BOOTP packet. */
...
)
{
	...
	struct option_cache *oc;
	struct data_string d1;
	int have_client_identifier = 0;			//是否有50号options设置uid
...
/****************************************************************************/
				   ...
				   &global_scope, oc, MDL)) 
	{
		...
		/* First, try to find a fixed host entry for the specified client identifier... */
		...
		find_lease_by_uid(&uid_lease, client_identifier.data, client_identifier.len, MDL);
	}
	...
	while (uid_lease) 
	{
		     ...
		     !permitted(packet, uid_lease->pool->permit_list))) 
		{
			...
			/* 若client没有希望使用的IP，那么销毁记录的之前这个client的lease */
			if (do_release)
	...
	while (hw_lease) 
	{
		...
		if (hw_lease->binding_state != FTS_FREE &&
		    ...
		    hw_lease->uid &&
		    /* 没有uid或者lease里面的uid和client的uid不一致 */
	...
	if (ip_lease && 
		(ip_lease->uid ? 
		/* 若有uid，比较uid和client中的是否一样 */
		(!have_client_identifier ||
	      ip_lease->uid_len != client_identifier.len ||
	      memcmp(ip_lease->uid, client_identifier.data, ip_lease->uid_len)) :
	      /* 没有uid，比较硬件地址和client中的是否一样 */
	     (ip_lease->hardware_addr.hbuf [0] != packet->raw->htype ||
	      ip_lease->hardware_addr.hlen != packet->raw->hlen + 1 ||
	      memcmp(&ip_lease->hardware_addr.hbuf[1], packet->raw->chaddr,
	...
	{
		/* 若ip_leaes和uid_lease不一样但是标识符一样，说明有很多uid_lease */
		if (have_client_identifier && (ip_lease->uid_len == client_identifier.len) &&
	...
	if (lease && host && !lease->host) 
	{
		...
		while (p != NULL) 
		{
			/* 若找到一个host没有fixed_addr，挂接给host，break */
			if (!p->fixed_addr) 
			{
				/*
...
Input		 :  IN struct lease ** lp
			    IN struct packet * packet
			    IN struct shared_network * share
			    IN struct host_decl * hp
Output		 : 
Return		 :  int
Caution 	 : 	Search the provided host_decl structure list for an address that's on
...
)
{
	...
	/* 寻找和share在一个网段的host，给subnet，rhp，ip_addr赋值 */
	...
	/* 其余字段初始化 */
	memcpy(lease->uid, rhp->client_identifier.data, rhp->client_identifier.len);
...
/*********************************************************************
Func Name 	 : allocate_lease
Date Created : 2018/06/05
Author		 : wangzhe
Description	 : 分配一个新的lease
Input		 : IN struct lease ** lp
			   IN struct packet * packet
			   IN struct pool * pool
			   IN int * peer_has_leases
Output		 : 
Return		 : 
Caution 	 : Look through all the pools in a list starting with the specified pool
...
Date Created :  2018/06/05
Author		 :  wangzhe
Description	 :  根据pakcet中的标记在permit列表中判断packet是否permit
Input		 :  IN struct * packet
				IN struct permit * permit_lise
Output		 : 
Return		 :  int
Caution 	 :  Determine whether or not a permit exists on a particular permit list
...
/*********************************************************************
Func Name :   locate_network
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  确定client在哪个网段，在这个网段给client分配IP
Input:	      IN struct packet * packet
Output:       None
Return:       void
Caution : 	  
...
)
{
	...
	/* 前者是client和server在不同网段，后者是在相同网段但是client想要不同网段的IP */
	/* See if there's a Relay Agent Link Selection Option, or a
	 * Subnet Selection Option.  The Link-Select and Subnet-Select
	 * are formatted and used precisely the same, but we must prefer
	 * the link-select over the subnet-select.
	...
	if (!oc && !packet->raw->giaddr.s_addr) 
	{
		...
		{
			/* 若不是中继代理，client和server在一个网段，挂接然后return */
			shared_network_reference(&packet->shared_network,
				 packet->interface->shared_network, MDL);
			return 1;
		}
...
/*********************************************************************
Func Name 	 : get_server_source_address
Date Created : 2018/06/05
Author		 : wangzhe
Description	 : 获取源IP地址
Input		 : IN struct in_addr * from
			   IN struct options_state * options
			   IN struct options_state * out_options
			   IN struct packet * packet
Output		 : 
Return		 : 
Caution 	 : 
*********************************************************************/
void get_server_source_address
(
	struct in_addr *from,
	struct option_state *options,
	struct option_state *out_options,
...
) 
{
	...
	if ((found == ISC_FALSE) && (packet->interface->address_count > 0)) 
	{
	...
	if ((a != NULL) && (option_cache_allocate(&oc, MDL))) 
	{
		if (make_const_data(&oc->expression,
...
/*********************************************************************
Func Name 	 : maybe_return_agent_options
Date Created : 2018/06/05
Author		 : wangzhe
Description	 : 
Input		 : IN struct packet * packet
...
Caution 	 : 
*********************************************************************/
static void maybe_return_agent_options
(
	struct packet *packet, 
	struct option_state *options
)
...
) 
{
    ...
    if ((lease->host && lease->host->name) &&
        ...
        (evaluate_boolean_option_cache(NULL, packet, lease, NULL,
				   packet->options, options,
				   &lease->scope,
				   lookup_option(&server_universe, options, ocode), MDL))) 
	{
		struct option_cache *oc = NULL;
        if (option_cache_allocate(&oc, MDL)) 
		{
                     ...
                     strlen(lease->host->name), 1, 0, MDL)) 
			{
				ocode = DHO_HOST_NAME;
                option_code_hash_lookup(&oc->option,
                                 dhcp_universe.code_hash, &ocode, 0, MDL);
                save_option(&dhcp_universe, options, oc);
            }
            option_cache_dereference(&oc, MDL);
        }
