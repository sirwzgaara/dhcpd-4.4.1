...
/*********************************************************************
Func Name :   dhcp_failover_startup
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  failover启动入口
Input:	      
Output:       
Return:       None
Caution : 
...
/*********************************************************************
Func Name :   enter_failover_peer
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  加载一个conf文件中解析到的failover配置，将failover_state加入链表管理
Input:	      
Output:       
Return:       isc_result_t
Caution : 
*********************************************************************/
isc_result_t enter_failover_peer
(
...
)
{
	...
	if (status == ISC_R_NOTFOUND) 
	{
		if (failover_states) 
		{
			dhcp_failover_state_reference(&peer->next, failover_states, MDL);
			dhcp_failover_state_dereference(&failover_states, MDL);
		}
...
Caution : 
*********************************************************************/
isc_result_t find_failover_peer
(
	dhcp_failover_state_t **peer,
	const char *name,
	const char *file,
...
isc_result_t dhcp_failover_link_initiate(omapi_object_t *h)
{
	...
	option_cache_reference(&obj->peer_address, state->partner.address, MDL);
	...
	/* Make an omapi address list out of a buffer containing zero or more IPv4 addresses. */
	/* ds中的len是字节数，除以4就是IP地址数量 */
	...
	/* failover only supports for ipv4 */
	for (i = 0; i < addrs->count; i++)
	{
		addrs->addresses[i].addrtype = AF_INET;
		addrs->addresses[i].addrlen = sizeof(struct in_addr);
		memcpy(addrs->addresses[i].address, &ds.data[i * 4], sizeof(struct in_addr));
				    ...
				    MDL)) 
	{
		...
		{
			log_fatal ("failover peer %s: no local address.", state->name);
		}
	}
	else 
	{
		...
		if (!state->server_identifier.len)
			data_string_copy(&state->server_identifier, &ds, MDL);
		data_string_forget(&ds, MDL);
		local_addr.port = 0;  /* Let the O.S. choose. */
	}

	status = omapi_connect_list((omapi_object_t *)obj, addrs, &local_addr);
	omapi_addr_list_dereference(&addrs, MDL);
...
/*********************************************************************
Func Name :   dhcp_failover_link_singal
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  处理网络连接事件
Input:	      
Output:       
Return:       isc_result_t
Caution : 
*********************************************************************/
isc_result_t dhcp_failover_link_signal 
(
	omapi_object_t *h,
	const char *name,
	va_list ap
)
{
	...
	/* 和peer建立TCP时，从connection对象发起信号，进入此分支 */
	if (!strcmp(name, "connect")) 
	{
	    ...
	    /* Allow the peer fifteen seconds to send us a startup message. */
#if defined (DEBUG_FAILOVER_TIMING)
	...
	if (!strcmp(name, "disconnect")) 
	{
		...
		{
			dhcp_failover_state_reference(&state, link->state_object, MDL);
	...
	if (!strcmp(name, "status")) 
	{
	  	if (link->state_object) 
		{
	    	...
	    	if ((status == ISC_R_HOSTUNREACH) || (status == ISC_R_TIMEDOUT)) 
			{
				...
				/* Make the transition. */
				dhcp_failover_state_transition(link->state_object, "disconnect");
	...
	switch (link->state) 
	{
	    ...
	    case dhcp_flink_message_length_wait:
			...
			if (!link->imsg) 
			{
				...
				link->state = dhcp_flink_disconnected;
				log_info("message length wait: %s", isc_result_totext(status));
			...
			if ((omapi_connection_require(c, link->imsg_len - 2U)) != ISC_R_SUCCESS)
				break;
...
# endif
			...
			if (link->imsg_payoff - link->imsg_count) 
			{
				omapi_connection_copyout((unsigned char *)0, c,
							  (link->imsg_payoff - link->imsg_count));
			...
			if (link->imsg->type == FTM_CONNECT) 
			{
			    ...
			    if (!state) 
				{
				  ...
				  badconnect:
				    ...
				    log_error("Failover CONNECT from %.*s: %s", slen, sname, errmsg);
...
)
{
	...
	{
		log_error("FAILOVER: invalid message type: %d", link->imsg->type);
		return DHCP_R_PROTOCOLERROR;
	}
	...
	else 
	{
		    ...
		    ft_options[option_code].type == FT_DDNS1) 
		{
			...
			op_count = (ft_options[option_code].type == FT_DDNS1 ? 1 : 2);

			omapi_connection_copyout(&ddns->codes [0], c, op_count);
...
isc_result_t dhcp_failover_link_set_value 
(
	omapi_object_t *h,
	omapi_object_t *id,
...
isc_result_t dhcp_failover_link_get_value 
(
	omapi_object_t *h,
	omapi_object_t *id,
	omapi_data_string_t *name,
...
isc_result_t dhcp_failover_link_destroy 
(
	omapi_object_t *h,
	char *file, 
	int line
)
{
	...
	if (link->state_object)
		dhcp_failover_state_dereference(&link->state_object, file, line);
...
)
{
	...
	if (link->inner && link->inner->type->stuff_values)
		return (*(link->inner->type->stuff_values))(c, id, link->inner);
	
	return ISC_R_SUCCESS;
}

/*********************************************************************
Func Name :   dhcp_failover_listen
Date Created: 2018/07/10
Author:  	  wangzhe
Description:  监听failover端口
Input:	      
Output:       
Return:       None
Caution : 	  Set up a listener for the omapi protocol.    The handle stored points to
...
isc_result_t dhcp_failover_listen(omapi_object_t *h)
{
	...
	/* get port, address, etc.*/
	
	/* dhcp_failover_state_get_value */
	status = omapi_get_value_str(h, (omapi_object_t *)0, "local-port", &value);
	if (status != ISC_R_SUCCESS)
		return status;
	...
	status = omapi_get_value_str(h, (omapi_object_t *)0, "local-address", &value);
	if (status != ISC_R_SUCCESS)
	...
	/* state->outer = listener, listener->inner = state */
	...
	/* Put this listener on the head of list. */
	if (failover_listeners) 
	{
		dhcp_failover_listener_reference(&obj->next, failover_listeners, MDL);
...
Output:       
Return:       None
Caution : 	  Signal handler for protocol listener - if we get a connect signal,
   create a new protocol connection, otherwise pass the signal down
*********************************************************************/
isc_result_t dhcp_failover_listener_signal 
(
	omapi_object_t *o,
	const char *name, 
	va_list ap
)
{
	...
	/* See if we can find a failover_state object that matches this connection. */
	for (s = failover_states; s; s = s->next) 
	{
	...
	status = omapi_object_reference(&c->inner, (omapi_object_t *)obj, MDL);
	if (status != ISC_R_SUCCESS)
		goto lose;

	status = dhcp_failover_state_reference(&obj->state_object, state, MDL);
	if (status != ISC_R_SUCCESS)
		goto lose;
...
/*********************************************************************
Func Name :   dhcp_failover_state_signal
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  Signal handler for protocol state machine
Input:	      
Output:       
Return:       None
...
)
{
	...
	else if (!strcmp(name, "message")) 
	{
		...
		if (link->imsg->type == FTM_CONNECT) 
		{
			...
			cancel_timeout(dhcp_failover_link_startup_timeout, link);
		}
		/* 收到ACK报文 */
		else if (link->imsg->type == FTM_CONNECTACK) 
		{
			...
			/* 收到ACK报文意味着TCP连接已经建立了，取消startup超时函数 */
		    cancel_timeout(dhcp_failover_link_startup_timeout, link);

		    if (!(link->imsg->options_present & FTB_RELATIONSHIP_NAME)) 
			{
				errmsg = "missing relationship-name";
				reason = FTR_INVALID_PARTNER;
				goto badconnectack;
		    ...
		    if (!dhcp_failover_state_match_by_name(state, &link->imsg->relationship_name)) 
			{
			...
			/* 建立TCP连接之后，master挂接state的link_to_peer接口 */
		    dhcp_failover_link_reference(&state->link_to_peer, link, MDL);
#if 0
...
#endif
	    lease_reference(&state->update_queue_tail, state->ack_queue_tail, MDL);
    }
...
/*********************************************************************
Func Name :   dhcp_failover_peer_state_changed
Date Created: 2018/07/17
Author:  	  wangzhe
Description:  收到FTM_STATE报文后调用此函数，处理对端状态变化
Input:	      
Output:       
Return:       isc_result_t
Caution : 	  
*********************************************************************/
isc_result_t dhcp_failover_peer_state_changed
(
	dhcp_failover_state_t *state,
...
`	/* 若未收到ack的lease到达一定数量，不再发送新的lease了 */
		...
		state->cur_unacked_updates) && state->update_queue_head) 
	{
		...
		if (lp->next_pending) 
		{
			lease_reference(&state->update_queue_head, lp->next_pending, MDL);
		...
		{
			lease_reference(&state->ack_queue_tail->next_pending, lp, MDL);
			lease_dereference(&state->ack_queue_tail, MDL);
		}
		else 
		{
...
/*********************************************************************
Func Name :   dhcp_failover_send_acks
Date Created: 2018/07/18
Author:  	  wangzhe
Description:  发送所有BNDACK到对端
Input:	      
Output:       
Return:       isc_result_t
Caution :     检查toack_queue，每个lease发送一个ACK消息
*********************************************************************/
int dhcp_failover_send_acks 
(
	dhcp_failover_state_t *state
)
{
	...
	{
		failover_message_reference(&msg, state->toack_queue_head, MDL);
		failover_message_dereference(&state->toack_queue_head, MDL);
		if (msg->next) 
		{
...
/*********************************************************************
Func Name :   dhcp_failover_queue_ack
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  缓存需要回复的BNDACK，根据缓存的数量立刻发送或者2秒后发送
Input:	      
Output:       
Return:       
Caution : 	  Queue an ack for a message.  There is currently no way to queue a
   negative ack -- these need to be sent directly
*********************************************************************/
int dhcp_failover_queue_ack 
(
	dhcp_failover_state_t *state,
	failover_message_t *msg
)
{
	...
	{
		failover_message_reference(&state->toack_queue_head, msg, MDL);
	}
...
/*********************************************************************
Func Name :   dhcp_failover_reconnect
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  failover重连函数
Input:	      
Output:       
Return:       None
Caution : 	  
...
/*********************************************************************
Func Name :   dhcp_failover_startup_timeout
Date Created: 2018/07/10
Author:  	  wangzhe
Description:  进入startup状态超时处理函数，超时时间是15秒
Input:	      
Output:       
Return:       None
Caution : 	  
...
/*********************************************************************
Func Name :   dhcp_failover_link_startup_timeout
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  连接创建超时处理
Input:	      
Output:       
Return:       None
Caution : 	  
...
/*********************************************************************
Func Name :   dhcp_failover_listener_restart
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  持续重启监听，间隔是90秒
Input:	      
Output:       
Return:       None
Caution : 
...
/*********************************************************************
Func Name :   dhcp_failover_state_get_value
Date Created: 2018/07/17
Author:  	  wangzhe
Description:  state对象的get_value函数
Input:	      
Output:       
Return:       None
Caution : 
*********************************************************************/
isc_result_t dhcp_failover_state_get_value 
(
	omapi_object_t *h,
	omapi_object_t *id,
	omapi_data_string_t *name,
...
/*********************************************************************
Func Name :   dhcp_failover_state_match
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  用partner的IP地址查找是否存在
Input:	      
Output:       
Return:       int
Caution : 
*********************************************************************/
int dhcp_failover_state_match 
(
	dhcp_failover_state_t *state,
	u_int8_t *addr, 
	unsigned addrlen
)
{
				   ...
				   state->partner.address, MDL))
	{
		...
		{
			if (!memcmp(&ds.data[i], addr, addrlen)) 
			{
				data_string_forget(&ds, MDL);
				return 1;
			}
...
/*********************************************************************
Func Name :   dhcp_failover_put_message
Date Created: 2018/07/17
Author:  	  wangzhe
Description:  Send a failover message header
Input:	      
Output:       
Return:       None
Caution : 	  
*********************************************************************/
isc_result_t dhcp_failover_put_message
(
	dhcp_failover_link_t *link,
...
)
{
	...
	while ((option = va_arg(list, failover_option_t *))) 
	{
		...
		if (!bad_option && opbuf)
			memcpy (&opbuf [opix], option->data, option->count);
		
		if (option != &null_failover_option && option != &skip_failover_option) 
		{
			opix += option->count;
			dfree (option->data, MDL);
			dfree (option, MDL);
	...
	if (link->state_object && link->state_object->link_to_peer == link) 
	{
#if defined (DEBUG_FAILOVER_CONTACT_TIMING)
		log_info ("add_timeout +%d %s",
...
#endif
		tv.tv_sec = cur_time +
			(int)(link->state_object-> partner.max_response_delay) / 3;
		tv.tv_usec = 0;
		add_timeout (&tv,
...
/*********************************************************************
Func Name :   dhcp_failover_send_contact
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  发送contact心跳
Input:	      
Output:       
Return:       None
Caution : 
...
/*********************************************************************
Func Name :   dhcp_failover_send_state
Date Created: 2018/07/10
Author:  	  wangzhe
Description:  发送状态到对端
Input:	      
...
/*********************************************************************
Func Name :   dhcp_failover_send_connext
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  Send a connect message, master to secondary
Input:	      
Output:       
Return:       None
...
# define FMA (char *)0, (unsigned *)0, 0
	...
	/* send connect message to secondary */
	status =
...
/*********************************************************************
Func Name :   dhcp_failover_send_connectack
Date Created: 2018/07/10
Author:  	  wangzhe
Description:  回复收到的connect消息, secondary to master
Input:	      
Output:       
Return:       None
Caution : 	  
*********************************************************************/
isc_result_t dhcp_failover_send_connectack 
(
	omapi_object_t *l,
	dhcp_failover_state_t *state,
	int reason, 
...
/*********************************************************************
Func Name :   dhcp_failover_send_bind_update
Date Created: 2018/07/18
Author:  	  wangzhe
Description:  Send a Bind Update message
Input:	      
Output:       
Return:       isc_result_t
Caution :     将发生状态变化的lease通知到对端
*********************************************************************/
isc_result_t dhcp_failover_send_bind_update 
(
	dhcp_failover_state_t *state,
	struct lease *lease
)
...
/*********************************************************************
Func Name :   dhcp_failover_send_bind_ack
Date Created: 2018/07/18
Author:  	  wangzhe
Description:  Send a Bind ACK message
Input:	      
Output:       
Return:       isc_result_t
Caution :     
...
)
{
	...
	if (msg->options_present & FTB_CHADDR)
	{
		...
		memcpy(lt->hardware_addr.hbuf, msg->chaddr.data, msg->chaddr.count);
	}
	 ...
	 * scope blindly.
	 */
	if (msg->binding_status == FTS_ACTIVE && (chaddr_changed || ident_changed)) 
	{
#if defined (NSUPDATE)
	...
	if (msg->options_present & FTB_IP_FLAGS) 
	{
		...
		if (msg->ip_flags & ~(FTF_IP_FLAG_RESERVE | FTF_IP_FLAG_BOOTP))
			log_info("Unknown IP-flags set in BNDUPD (0x%x).", msg->ip_flags);
	}
	...
	if (!supersede_lease(lease, lt, 0, 0, 0, 0) || !write_lease (lease)) 
	{
...
/*********************************************************************
Func Name :   dhcp_failover_process_bind_ack
Date Created: 2018/07/10
Author:  	  wangzhe
...
Caution : 	  
*********************************************************************/
isc_result_t dhcp_failover_process_bind_ack 
(
	dhcp_failover_state_t *state,
	failover_message_t *msg
)
...
binding_state_t conflict_binding_state_transition_check 
(
	struct lease *lease,
	dhcp_failover_state_t *state,
	binding_state_t binding_state,
