...
/*********************************************************************
Func Name :   lc_not_empty
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  判断是否是空的
Input:	      struct leasechain * lc
Output:       
Return:       int
Caution : 
*********************************************************************/
...
/*********************************************************************
Func Name :   lc_get_first_lease
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  找第一个lease的指针
Input:	      struct leasechain * lc
Output:       
Return:       int
Caution : 
*********************************************************************/
struct lease * lc_get_first_lease
(
	struct leasechain *lc
) 
{
...
/*********************************************************************
Func Name :   lc_binary_search_insert_point
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  以sort_time为key二分查找lp的插入位置
Input:	      struct leasechain * lc
			  struct lease * lp
			  size_t min
			  size_t max
Output:       
Return:       size_t
Caution : 	  若min符合，直接在min插入，若max符合，需要给max+1
*********************************************************************/
size_t lc_binary_search_insert_point
(
	struct leasechain *lc,
    struct lease *lp,
	size_t min, 
...
)
{
	     ...
	     (lc->list[mid_index]->sort_tiebreaker > lp->sort_tiebreaker))) 
	{
		...
		return (lc_binary_search_insert_point(lc, lp, min, mid_index - 1));
	}
		     ...
		     (lc->list[mid_index]->sort_tiebreaker < lp->sort_tiebreaker))) 
	{
		...
		return (lc_binary_search_insert_point(lc, lp, mid_index + 1, max));
	}
...
/*********************************************************************
Func Name :   lc_binary_search_lease
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  二分查找
Input:	      struct leasechain * lc
...
)
{
	...
	/* 先找mid_index左边的，即到min，然后找mid_index右边的，即到max */
...
/*********************************************************************
Func Name :   lc_link_lcp
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  将lp插入lc链表的第n个格子
Input:	      struct leasechain * lc
			  struct lease *lp
			  size_t n
Output:       
...
) 
{
	...
	if (n < lc->nelem) 
	{
		...
		log_debug("LC link lcp moving position %zu, moving %zu. %s:%d",
			  n, (lc->nelem-n), MDL);
#endif
		/* 这个相当于从n开始往后挪，挪出来一个格子n */
		memmove(lc->list + n + 1,  lc->list + n, sizeof(struct lease *) * (lc->nelem - n));
	}
...
/*********************************************************************
Func Name :   lc_add_lease_pos
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  在第pos位置挂接节点到链表
Input:	      struct leasechain * lc
			  struct lease *lp
			  size_t pos
Output:       
...
/*********************************************************************
Func Name :   lc_check_lc_sort_order
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  检查链表是否是按照顺序排序的，若不是，打印日志
Input:	      struct leasechain * lc
Output:       
Return:       void
Caution : 	  
...
/*********************************************************************
Func Name :   lc_add_sorted_lease
Date Created: 2018/05/31
Author:  	  wangzhe
Description:  将lp插入lc链表中
Input:	      struct leasechain * lc
			  struct lease * lp
Output:       
Return:       void
...
) 
{
	...
	else if (lp->sort_time == lc->list[lc->nelem - 1]->sort_time) 
	{
		...
		if (lc->list[lc->nelem - 1]->sort_tiebreaker < LONG_MAX)
			lp->sort_tiebreaker = lc->list[lc->nelem - 1]->sort_tiebreaker + 1;
		else
			lp->sort_tiebreaker = LONG_MAX;
		pos = lc->nelem;
	} 
	/* 若都不是，副key设置为随机数，寻找插入位置pos */
	else 
	{
...
Author:  	  wangzhe
Description:  将第n个节点从数组删掉，内部函数，外层保证n是合法的
Input:	      struct leasechain * lc
			  size_t n
Output:       
Return:       void
...
) 
{
	...
	if ((lc->nelem - 1) > n) 
	{
		memmove(lc->list + n, lc->list + n + 1, sizeof(struct lease *) * (lc->nelem - 1 - n));
	}
	lc->nelem--;
}
...
/*********************************************************************
Func Name :   lc_unlink_lease_pos
Date Created: 2018/06/01
Author:  	  wangzhe
Description:  删除pos位置的节点
Input:	      struct leasechain * lc
			  size_t pos
Output:       
Return:       void
...
/*********************************************************************
Func Name :   lc_unlink_lease
Date Created: 2018/06/01
Author:  	  wangzhe
Description:  将lp从链表和数组中删除
Input:	      struct leasechain * lc
			  struct lease * lp
Output:       
Return:       void
...
) 
{
	...
	if (pos == SIZE_MAX) 
	{
		...
		log_fatal("Lease with binding state %s not on its queue.",
			  (lp->binding_state < 1 || lp->binding_state > FTS_LAST)
			  ? "unknown" : binding_state_names[lp->binding_state - 1]);
	}
