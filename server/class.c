...
void classification_setup ()
{
	...
	if (!executable_statement_allocate (&default_classification_rules, MDL))
	...
	if (!expression_allocate (&default_classification_rules->data.eval, MDL))
...
/*********************************************************************
Func Name :   find_class
Date Created: 2018/06/04
Author:  	  wangzhe
Description:  查找和name匹配的class，挂接给class指针
Input:	      IN struct class **class
			  IN const char * name
			  IN cosnt char * file
			  IN int line
Output:       
