...
Author:  	  wangzhe
Description:  读取dhcpd.conf
Input:	      None
Output:       None
Return:       
Caution : 	  conf-file :== parameters declarations END_OF_FILE
...
isc_result_t readconf()
{
	...
	/* 在main中，若发现-cf，path_dhcpd_conf就是配置中指定的路径 */
...
/*********************************************************************
Func Name :   read_conf_file
Date Created: 2018/05/28
Author:  	  wangzhe
Description:  读取dhcpd.conf
Input:	      IN const char * filename
			  IN struct group * group
			  IN int group_type
			  IN int leasep				标记解析lease还是conf
Output:       None
Return:       
Caution : 	 
*********************************************************************/
isc_result_t read_conf_file
(
	const char *filename, 
	struct group *group,
	int group_type, 
...
/*********************************************************************
Func Name :   conf_file_subparse
Date Created: 2018/05/28
Author:  	  wangzhe
Description:  解析conf文件core函数
Input:	      IN struct parse * cfile  文件映射到内存中的起始地址
			  IN struct group * group
			  IN int group_type
Output:       None
Return:       
Caution : 	  conf-file :== parameters declarations END_OF_FILE
   			  ...
   			  declarations :== <nil> | declaration | declarations declaration
*********************************************************************/
isc_result_t conf_file_subparse 
(
	struct parse *cfile, 
	struct group *group,
	int group_type
	...
	switch (token) {
	   ...
	   case FP_ENABLE:
		...
		return 1;
	      ...
	      case SUBNET:
	      case SUBNET6:
		/* peek_token的时候不消耗符号，这里要将符号跳过 */
		skip_token(&val, (unsigned *)0, cfile);
		if (type == HOST_DECL || type == SUBNET_DECL || type == CLASS_DECL) 
		{
			parse_warn(cfile, "subnet declarations not allowed here.");
			skip_to_semi(cfile);
			return 1;
		}
		...
		/* 给share->group分配内存，并将group到share->group->next */
		...
		/* This is an implicit shared network, not explicit in the config. */
		/* 标记这个share是软件产生的 */
		...
		et = (struct executable_statement *)0;
		lose = 0;
		if (!parse_executable_statement(&et, cfile, &lose, context_any)) 
		{
			if (!lose) 
			{
				if (declaration)
					parse_warn(cfile, "expecting a declaration");
				else
					parse_warn(cfile, "expecting a parameter %s or declaration");
				skip_to_semi(cfile);
			}
...
/*********************************************************************
Func Name :   parse_failover_peer
Date Created: 2018/07/09
Author:  	  wangzhe
Description:  解析failover配置
Input:	      
Output:       
Return:       void
Caution : 
...
)
{
	...
	/* 解析大括号内部的配置 */
	do {
		/* cp是本端或者对端，复用了变量，若遇到peer，cp就是对端的 */
		cp = &peer->me;
	      peer:
		token = next_token(&val, (unsigned *)0, cfile);
		switch (token) 
...
/*********************************************************************
Func Name :   dhcp_failover_state_decalration
Date Created: 2018/07/10
Author:  	  wangzhe
Description:  解析conf文件中failover参数
Input:	      
Output:       
Return:       None
Caution : 
*********************************************************************/
void parse_failover_state_declaration 
(
	struct parse *cfile,
	dhcp_failover_state_t *peer
)
...
Caution : 	  
*********************************************************************/
static int common_subnet_parsing
(
	struct parse *cfile, 
	struct shared_network *share,
	...
	else 
	{
		...
		for (t = share->subnets; t->next_sibling; t = t->next_sibling) 
		{
			if (subnet_inner_than(subnet, t, 0)) 
			{
				...
				if (u) 
				{
					subnet_dereference(&u->next_sibling, MDL);
					subnet_reference(&u->next_sibling, subnet, MDL);
				} 
				else 
				{
					subnet_dereference(&share->subnets, MDL);
					subnet_reference(&share->subnets, subnet, MDL);
				}
				subnet_dereference(&subnet, MDL);
				return 1;
...
)
{
	...
	/* omapip.h OMAPI_OBJECT_ALLOC */
	status = subnet_allocate(&subnet, MDL);		//omapi_object_allocate
	if (status != ISC_R_SUCCESS)
		log_fatal("Allocation of new subnet failed: %s", isc_result_totext(status));
	...
	/* sunbet后面跟的IP地址会读到addr中，保存在subnet->net中 */
	...
	if (host_addr(subnet->net, subnet->netmask)) 
	{
		...
		if (maskstr == NULL) {
			log_fatal("Allocation of subnet maskstr failed: %s", piaddr(subnet->net));
		}
...
/*********************************************************************
Func Name :   parse_address_range
Date Created: 2018/06/06
Author:  	  wangzhe
Description:  解析conf文件中subnet选项下的range
Input:	      
Output:       
Return:       
Caution : 	  address-range-declaration :== ip-address ip-address SEMI
			       | DYNAMIC_BOOTP ip-address ip-address SEMI
*********************************************************************/
void parse_address_range
(
	struct parse *cfile,
	struct group *group,
	int type,
...
)
{
	...
	/* 检查是否有dynamic选项 */
	if ((token = peek_token(&val, (unsigned *)0, cfile)) == DYNAMIC_BOOTP) 
	{
		skip_token(&val, (unsigned *)0, cfile);
		dynamic = 1;
	}
