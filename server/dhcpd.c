...
/**************************************************************************/
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp (argv [i], "-f")) {
			...
			IGNORE_RET(write(STDERR_FILENO, vstring, strlen(vstring)));
			IGNORE_RET(write(STDERR_FILENO, PACKAGE_VERSION, strlen(PACKAGE_VERSION)));
			...
			IGNORE_RET(write(STDERR_FILENO, DHCPD_USAGE0, strlen(DHCPD_USAGE0)));
			IGNORE_RET(write(STDERR_FILENO, DHCPD_USAGE1, strlen(DHCPD_USAGE1)));
#if defined (PARANOIA)
	...
	if (daemon) 
	{
		...
		if (pid != 0) 
		{
			...
			do 
			{
				...
				if (n == 1)
					_exit((int)buf);
			/* EINTR是4，Interrupted system call */
...
/**********************************************************************/
	...
	if (status != ISC_R_SUCCESS)
		log_fatal("Can't initialize context: %s", isc_result_totext(status));
...
/*********************************************************************/
	...
	result = omapi_init();
	if (result != ISC_R_SUCCESS)
		log_fatal ("Can't initialize OMAPI: %s", isc_result_totext (result));
...
/**************************** 数据库内部对象设置OMAPI封装器 ****************************/
/***************************************************************************************/
	/* Set up the OMAPI wrappers for various server database internal objects. */
	dhcp_common_objects_setup();

	/* Initially, log errors to stderr as well as to syslogd. */
	openlog (isc_file_basename(progname), DHCP_LOG_OPTIONS, DHCPD_LOG_FACILITY);
...
/**************************************************************************/
	...
	for (i = 1; i < argc; i++) 
	{
		if (!strcmp (argv [i], "-p")) {
			...
			log_debug ("binding to user-specified port %d", ntohs (local_port));
...
/**********************************************************************/
	...
	{
		int junk;
		memcpy(&junk, &ip->hw_address.hbuf[ip->hw_address.hlen - sizeof(seed)], sizeof(seed));
		seed += junk;
	}
...
/******************************************************************************/
	 ...
	 * appropriate.
	 */
	/* no_pid_file初始值是false，若解析参数发现没有pid，那么赋值为true */
...
/* 这个走不进去 */
	...
	if (daemon) 
	{
		...
		/* /dev/null这是个空洞文件，任何日志都不会显示，close之后open自动打开就是012 */
...
/*********************************************************************
Func Name 	 : lease_pinged
Date Created : 2018/06/14
Author		 : wangzhe
Description	 : ping包没超时，说明IP有人在用，不能分配出去，调用这个函数
Input		 : IN struct iaddr from
			   IN u_int_8 * packet
			   IN int length
Output		 : 
Return		 : 
...
)
{
	...
	/* 应该有和ip地址对应的lease，否则有bug */
	lp = (struct lease *)0;
	if (!find_lease_by_ip_addr(&lp, from, MDL)) 
	{
		log_debug("unexpected ICMP Echo Reply from %s", piaddr(from));
		return;
	}
...
#endif
			log_debug("ICMP Echo Reply for %s late or spurious.", piaddr(from));
		goto out;
	}
	...
	if (lp->ends > cur_time) 
	{
		log_debug("ICMP Echo reply while lease %s valid.", piaddr(from));
	}
...
/*********************************************************************
Func Name 	 : lease_ping_timeout
Date Created : 2018/06/14
Author		 : wangzhe
Description	 : ping包超时，说明IP没人使用，可以分配，调用这个函数
Input		 : void *vlp
Output		 : 
Return		 : 
Caution 	 : 
