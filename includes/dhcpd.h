...
struct option_cache {
	...
	struct expression *expression;      //defined in tree.h
	struct option *option;				//defined in tree.h
...
struct lease_state {
	...
	int max_message_size;		//最大message长度
	unsigned char expiry[4], renewal[4], rebind[4];		//到期时间，50%时间，87.5%时间
	...
	u_int32_t xid;				//client提供的随机数
	u_int16_t secs;				//client启动时间
	u_int16_t bootp_flags;		//最左bit置位说明是广播
	struct in_addr ciaddr;		//client希望使用的IP地址
	struct in_addr siaddr;		
	struct in_addr giaddr;		//中继代理IP地址
	u_int8_t hops;
	u_int8_t offer;
	struct iaddr from;
};
...
struct subnet {
	...
	struct iaddr net;		//ip地址
	struct iaddr netmask;	//掩码
